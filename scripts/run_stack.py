...
退出语义：
  有显示窗口（--viewer / --cams）→ 不限拍数，任务结束后窗口保持打开，
  手动关闭全部显示窗口（或 Ctrl+C）后退出；
  无显示窗口 → 任务终态即退出，--ticks 为拍数上限（CI/无头）。
"""
...
def main():
    ...
    modes = " ".join(filter(None, [
        ...
        "主窗口" if viewer else "",
        "相机窗口" if cams else ""]))
    tail = ("关闭全部显示窗口后退出" if (viewer or cams)
            else f"拍数: {args.ticks}")
    print(f"========== 任务: {args.task}（{TASKS[args.task]['desc']}）"
          f" | {tail} | {modes} ==========")
    # 有显示窗口：不限拍数，任务结束后窗口保持打开，用户手动关闭后退出；
    # 无显示窗口：任务终态即退出，--ticks 为拍数上限
